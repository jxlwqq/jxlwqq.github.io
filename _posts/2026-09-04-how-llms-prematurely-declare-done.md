...
> **⚠️ 适用范围**：本文解剖的失败模式和五个成因，主要来自**早期模型 + 朴素 harness** 组合下的观察。当前的 SOTA 模型配合成熟的工程实践（验证结果回灌、完成动作工具化、面向长程任务的推理训练），已经能够胜任相当长的长程任务——Claude Code、Codex 这类产品就是例证。但这不意味着本文的分析过时了：**EOS 的底层机制没有变**，变的是第三节那些对策被系统性地做进了模型训练和 harness 设计里。理解失败机制，恰恰是理解这些对策为什么有效的前提。
